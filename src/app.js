/* ============================================================
   API SERVICE LAYER & LOCAL DATABASE
============================================================ */
// Fungsi untuk nyimpen data permanen ke browser
function saveToStorage() {
  localStorage.setItem('hos_products', JSON.stringify(APP.products));
}
...
const API = {
  ...
  async getProducts() {
    // Cek apakah udah ada data di database lokal
    const stored = localStorage.getItem('hos_products');
    if (stored) {
      return { success: true, data: JSON.parse(stored) };
    } else {
      // Kalau belum ada, pakai data bawaan lalu simpan
      const mock = getMockProducts();
      localStorage.setItem('hos_products', JSON.stringify(mock));
  ...
  async createProduct(data) {
    return new Promise(resolve => setTimeout(() => {
      ...
      APP.products.push(p);
      saveToStorage(); // Simpan ke database
